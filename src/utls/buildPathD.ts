...
const buildPathD = (
  ...
  dataSlice.reduce((acum, item, i) => {
    ...
    let resultingFlexure = config[dataKey] && config[dataKey].flexure || flexure
    ...
    const lastSlice =
      i === dataSlice.length - 1
        ...
        ? `c ${delta * deltaSmall} 0 ${delta * deltaLarge} ${y - prevY} ${x - prevX} ${y - prevY} H ${x + reservedStrokeWidth} V ${SVGHeight + reservedStrokeWidth} H 0`
        : undefined
